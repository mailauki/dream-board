    ...
    <div className="flex max-w-xl flex-col items-start justify-between rounded border p-2">
      <Link href={`/${profile?.username}/dreams/${data.id}`}>
        <div className="relative flex w-full items-center gap-x-4">
          <img
            alt="Link Preview"
            className="h-60 w-full object-cover rounded bg-accent border"
            src={meta.image}
          />
        </div>
        <div className="group relative">
          <h3 className="mt-3 text-lg/6 font-semibold text-accent-foreground">
            {data.title || meta.title}
          </h3>
          <p className="mb-8 line-clamp-3 text-sm/6 text-muted-foreground">{data.description || meta.description}</p>
        </div>
      </Link>
      <div className="w-full flex justify-between items-center gap-x-4 text-xs">
        ...
        <Button asChild className='h-11' size={'sm'} variant={'default'}>
          <Link href={data.url || meta.baseURL} target="_blank">
            <span className='md:hidden'>Where to buy</span>
            <span>
